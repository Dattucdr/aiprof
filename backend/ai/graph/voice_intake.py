...
def create_voice_intake_state(patient_id: int, call_id: int, queue_item_id: int):
    return {
...
def build_voice_intake_graph():
    ...
    graph.add_node("red_flags", red_flag_node)
    graph.add_node("medication", medication_node)
    graph.add_node("patient_questions", patient_questions_node)
    graph.add_node("complete", complete_node)

    graph.add_edge(START, "introduction")
    graph.add_edge("introduction", "verify_patient")
    graph.add_edge("verify_patient", "wellbeing")
    graph.add_edge("wellbeing", "symptoms")
    graph.add_edge("symptoms", "red_flags")
    graph.add_edge("red_flags", "medication")
    graph.add_edge("medication", "patient_questions")
    graph.add_edge("patient_questions", "complete")
...
"""
    prompt = system_prompt.format(step=state["current_step"])
