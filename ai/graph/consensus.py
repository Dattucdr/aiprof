...
def build_disagreement_reason(
    ...
    if assessment_a.risk_level != assessment_b.risk_level:
        reasons.append(
            f"Risk disagreement: A={assessment_a.risk_level}, "
            f"B={assessment_b.risk_level}"
        )

    if assessment_a.recommended_action != assessment_b.recommended_action:
        reasons.append(
            f"Action disagreement: A={assessment_a.recommended_action}, "
            f"B={assessment_b.recommended_action}"
        )

    red_flags_a = {
        flag.lower().strip()
        for flag in assessment_a.red_flags
        if flag.strip()
    }

    red_flags_b = {
        flag.lower().strip()
        for flag in assessment_b.red_flags
        if flag.strip()
    }
...
def run_consensus(
    ...
    # ---------------------------------------------------------
    # 1. Validate both independent assessments
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # 2. Merge evidence and red flags
    # ---------------------------------------------------------

    merged_evidence = merge_unique(
        assessment_a.evidence,
        assessment_b.evidence,
    )

    merged_red_flags = merge_unique(
        assessment_a.red_flags,
        assessment_b.red_flags,
    )

    # ---------------------------------------------------------
    # 3. Determine whether assessments agree
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # 4. Select conservative risk/action
    # ---------------------------------------------------------

    final_risk_level = get_higher_risk(
        assessment_a.risk_level,
        assessment_b.risk_level,
    )

    final_action = get_higher_action(
        assessment_a.recommended_action,
        assessment_b.recommended_action,
    )

    # ---------------------------------------------------------
    # 5. Apply deterministic red-flag safety backstop
    # ---------------------------------------------------------
    ...
    # ---------------------------------------------------------
    # 6. Build final consensus result
    # ---------------------------------------------------------
