...
def validate_triage_result(result: TriageResult) -> TriageResult:
    ...
    if result.recommended_action not in ALLOWED_ACTIONS:
        raise ValueError(
            f"Invalid recommended action: {result.recommended_action}"
        )
...
def perform_triage(
    ...
    prompt = f"""
You are a clinical triage support component in a
post-discharge healthcare outreach system.
...
- If important information is uncertain, preserve that uncertainty.
- Red flags must remain explicitly represented.
...
Use the supplied hospital protocol as an operational constraint.

Do not override it.

If the protocol requires clinical review or escalation for
a documented condition or red flag, preserve that requirement.

If the protocol does not provide enough information,
